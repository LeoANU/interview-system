...
public class SearchServlet2 extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) //
		...
		try {
			 ...
			 String sql="SELECT interviewer,candidate,email,date,start,end,location,id,resume,comment from process where interviewer like'%*"+uname+"%'and(candidate like'%"+key+"%' or email like'%"+key+"%'or date like'%"+key+"%'or location like'%"+key+"%'or comment like'%"+key+"%') ";
		...
		request.getRequestDispatcher("/SHome2.jsp").forward(request, response);
